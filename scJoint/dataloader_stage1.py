...
class PrepareDataloader:
    def __init__(self, config) -> None:
        ...
        self.train_rna_loaders = []
        self.test_rna_loaders = []
        
        self.train_atac_loaders = []
        self.test_atac_loaders = []
        # ------------------------------ load RNA data -------------------------
        ...
        if len(config.atac_paths) == len(config.atac_protein_paths):
            for atac_path, atac_protein_path in zip(config.atac_paths, config.atac_protein_paths):  
                ...
                trainloader = torch.utils.data.DataLoader(trainset, batch_size=
                                config.batch_size, shuffle=True, **kwargs)                        
                self.train_atac_loaders.append(trainloader)
...
if __name__ == "__main__":
    ...
    train_rna_loaders, test_rna_loaders, train_atac_loaders, test_atac_loaders, temp = PrepareDataloader(config).getloader()

    for step, (x, y) in enumerate(train_rna_loaders[0]):
