...
def reduction_loss(embedding, identity_matrix, size):
    ...
    NNDR loss
    """
    # torch.triu(cor(embedding), diagonal=1)返回上三角矩阵，其他部分为0
    # calculte the mean of correlation of embedding
    # 计算每列之间的相关性，并取均值, 
    # the second term of NND loss function
    loss = torch.mean(torch.abs(torch.triu(cor(embedding), diagonal=1)))

    # 计算每个细胞的emdedding到几何中心的距离，并取均值
    # torch.mean(torch.abs(embedding - torch.mean(embedding, dim=0).view(1, size).repeat(embedding.size()[0], 1)))
    # the first term of NNDR looss function
...
class EncodingLoss(nn.Module):
    def __init__(self, dim=64, p =0.8, use_gpu = True):
        ...
        if use_gpu:
            # np.identity(dim) 创建一个dim x dim 的矩阵，对角线为1, 其他部分为0
